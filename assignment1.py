class Graph:
    ...
    def __init__(self, V):
        ...
        Function Description:
          Initializes a graph object with a list of vertices and sets the cycle_time and intercept attributes to None.
          cycle_time is the time for the friend to complete a loop/cycle, intercept is a tuple that is later used
        ...
        * L = number of vertices/locations
        ...
        Aux Space Complexity: O(L)
          Aux space complexity comes from us initializing an empty list of length L to store vertices, taking up O(L) space for auxiliary space.
    ...
    def reset_vertices(self):
        ...
        * C = cycle_time/number of layers, L = number of locations
        Time Complexity: O(L)
          ...
          which can be simplified to O(L) due to C being treated as a constant (upper boundary of 100).
        ...
        Aux Space Complexity: O(CL) = O(L)
          ...
          to store the vertices in each layer. However, since C is treated as a constant, we can simplify this to O(L) aux space complexity.
        
        Total Space Complexity: O(L) + O(L) = O(L)
          Since we are accessing the self.vertices array which has a size of L, we are treating this as our input space complexity. When we add
          this with our aux space complexity of O(L), we get a total space complexity of O(L + L) which can be simplified to O(L).
        """
    ...
    def dijkstra(self, source):
        ...
        Approach Description:
          ...
          * L = locations, R = roads, C = cycle_time/number of layers
          ...
          4.  Since a vertex can have different states in different layers, this effectively allows us to revisit a location, just in different layers.
        ...
        * L = locations, R = roads, C = cycle_time/number of layers
        Time Complexity: O(R log L)
          Time complexity of our Dijkstra stems from several different operations, which we will break down in steps:
            ...
            3.  Doing edge relaxation for every vertex we visit, where we will have O(C * R * log(C * L))) time complexity since we have to account
                for our multilayered graph and the cost of minheap operations (add, sink, rise, update). We are taking the worst case where each edge
                relaxation is O(log(C * L)) because we update the vertex in the heap.
        ...
        while len(min_heap) > 0:
            ...
            if original_vertex.is_station and u.time % self.cycle_time == original_vertex.time_from_friend_start:
...
class Vertex:
    ...
    def __init__(self, id):
        ...
        Aux Space Complexity: O(1)
          Only addtional constant space is required to store the values when initializing the vertex object, O(1) aux space complexity.
    ...
    def add_edge(self, edge):
        ...
        Aux Space Complexity: O(1)
          Only a constant amount of additional space is required in the function, O(1) aux space complexity.
        
        * R = number of roads/edges in the graph 
...
class Edge:
    ...
    def __init__(self, u, v, cost, time):
        ...
        Total Space Complexity: O(1)
          ...
          constant amount of addtional, giving us aux space complexity of O(1). Furthermore,
          input are also primitive values, so input space comp is O(1). This gives us total space comp of O(1)
        """
        self.u = u
        self.v = v
        self.cost = cost
...
class MinHeap:
    ...
    NOTE: While N = number of elements in the heap in the analysis, in the context of the assignment N is interchangeable with
    L = number of locations in the graph, as the heap is used to store the vertices in the graph. this means that O(N) = O(L).
    """
    def __init__(self, max_length):
        """
        Function Description:
          Initializes a MinHeap object with a maximum length given as an input and creates an
          empty array of the given max length+1.
        ...
        * N = max_length
        Time Complexity: O(N)
          The time complexity of this function comes from initializing an empty array of length max_length, where
          we require a time of O(N) to create the array of length N
        
        Total Space Complexity: O(N)
          Given that the input is simply an integer, our input space complexity is O(1). For our aux space complexity,
          we are creating an array of length max_length, so our aux space complexity is O(N). This gives us a total
    ...
    def rise(self, i):
        ...
        * N = number of elements in the heap
        Time Complexity: O(log N)
          The time complexity of this function is O(log N) since worst case we are rising the element from the bottom
          of the heap to the top of the heap. This means that we need to perform log N swaps to get the element
          to its correct position.
        
        Aux Space Complexity: O(1)
          Only a constant amount of additional space is required, O(1) aux space complexity.
        
        Total Space Complexity: O(N)
          The total space complexity of the function is O(N) from aux + input, where the array of the heap is of size N.
          This acts as the input so we will have space complexity of O(N + 1) or O(N) when simplified.
        """
        item = self.array[i]
    ...
    def sink(self, i):
        ...
        * N = number of elements in the heap
        Time Complexity: O(log N)
          The time complexity of this function is O(log N) worst case since we may potentially be sinking an element at
          the root of the heap to the bottom of the heap. This means that we need to perform log N swaps to get the element
          to its correct position.
        
        Aux Space Complexity: O(1)
          Only a constant amount of additional space is required for the function, giving us O(1) aux space complexity.
        
        Total Space Complexity: O(N)
          The total space complexity of the function is O(N) from aux + input, since the array of the heap is of size N.
          This array actas as the input so we will have a total space complexity of O(N + 1) or O(N) when simplified.
        """
        item = self.array[i]
    ...
    def update_node(self, node, new_cost) -> None:
        ...
        * N = number of elements in the heap
        Time Complexity: O(log N)
          ...
          bottom of the heap and rising it all the way up to the root. This has a complexity of O(log N).
        
        Aux Space Complexity: O(1)
          Only an additional constant amount of space is required, giving us O(1) aux space complexity.
        
        Total Space Complexity: O(N)
          The total space complexity of the function is O(N) from aux + input, since we are interacting with the array of the heap
          to get the node we have to update and rising the node. This array acts as the input so our total space complexity
          is O(N + 1) or O(N) simplified.
        """
        # Updating the cost of the node in the heap and rising it to its correct position
    ...
    def add(self, element) -> bool:
        ...
        * N = number of elements in the heap
        Time Complexity: O(log N)
          The time complexity of this function is O(log N) since we are adding an element to the bottom of the heap and rising it
          to its correct position. If the item happens to be the smallest element in the heap, we will have to rise it all the way
          to the root, taking log N swaps, this gives us O(log N) time complexity.

        Aux Space Complexity: O(1)
          Only an additional constant amount of extra space is required, giving us O(1) aux space complexity.
        
        Total Space Complexity: O(N)
          The total space complexity of the function is O(N) from aux + input, since we are interacting with the array of the heap
          we will be required to take into account the space taken by that array, giving is O(N) total space complexity.
        """
        if self.is_full():
            raise IndexError
    ...
    def smallest_child(self, i):
        """
        Function Description:
          Returns the index of the smallest child of a given node in the heap
        
        Input:
          i: an integer, the index of the node to get the smallest child of
        
        Returns:
          An integer, the index of the smallest child of the given node
        
        * N = number of elements in the heap
        Time Complexity: O(1)
          The function accesses the array to get the children of the node and compares them, which is O(1) time complexity.
          In the context of the assignment, the children would be vertices in the graph, and comparisons are done with O(1)
          time complexity since we only need to compare the cost and time which are both integers.
        
        Aux Space Complexity: O(1)
          Only a constant amount of additional space is required, O(1) space complexity.
        
        Total Space Complexity: O(N)
          Since we are accessing the array of the heap to get the children of the node, this array acts as our input.
          This gives us a complexity of O(N) since the array has size of N.
        """
    ...
    def get_min(self):
        ...
        Returns:
          a Vertex object with the minimum cost to reach
        
        * N = number of elements in the heap
        Time Complexity: O(log N)
          The time complexity of this function is O(log N) since we are swapping the root with the last element in the heap, then
          sinking it down to reach its correct position. If the element happens to be the largest element in the heap, we will
          have to sink it all the way back down to the bottom of the heap, taking log N swaps meaning O(log N) time complexity.
        
        Aux Space Complexity: O(1)
          Only a constant amount of additional space is required, giving us O(1) aux space complexity.
        
        Total Space Complexity: O(N)
          The total space complexity of the function is O(N) from aux + input, since we are interacting with the array of the heap
          we will need to take into account the space taken by that array which is O(N) space complexity.
        """
        if self.length == 0:
            raise IndexError
    ...
    def __len__(self):
        ...
        Total Space Complexity: O(1)
          Only a constant amount of addtional space is required to return the length of the heap, with no additional required for
          input, this gives us O(1) total space complexity.
        """
        return self.length
    ...
    def is_full(self):
        ...
        Total Space Complexity: O(1)
          Only a constant amount of additional space is required to return the boolean value, and no additional required space for input,
          so this gives us O(1) total space complexity.
        """
...
def preprocess(roads, stations, friendStart):
    ...
    Time Complexity: O(2R + 2S + L) = O(R)
      ...
      * R = number of roads, S = number of stations, L = number of locations
      ...
      Total time complexity of O(R + S + L + R + S) from finding max_loc_id, processing cycle time, initializing the graph, 
      adding graph edges, and adding station information.
      
      This gives us a complexity of O(2R + 2S + L), but since we know that R >= L >= S and S has an upperbound of 20, we can 
      simplify this to O(R) time complexity.
    ...
    my_graph = Graph([i for i in range(max_loc_id + 1)]) # O(L) for graph init
    ...
    for i in range(len(stations)): # O(S) time comp
        ...
        my_graph.vertices[station_index[0]].time_from_friend_start = time_taken
        # time taken equals to time it takes to reach from friendStart, used to check if can intercept
...
def intercept(roads, stations, start, friendStart):
    """
        ...
        Approach Description: 
          ...
          * R = number of roads, S = number of stations, L = number of locations
          ...
          1. After receiving the input, we will preprocess the input to get information such as:
            ...
            1.4 Adding extra information like time_taken to get to the station from friendStart and setting is_station to True. This is used
                to determine our interception point with the friend.
          
          2. After preprocessing, we will run a modified Dijkstra algorithm which will do the following steps:
            2.1 Resetting the vertices, where this is done by duplicating our vertices and storing them in a list of layers. Each layer will hold
                its own set of vertice, which will be used to store the state of the vertex in that layer.
            2.2 Running Dijkstra's algorithm from the starting location and traversing through the graph, the main difference is that every step
                we take, we will move to a different layer based on the time taken to reach that vertex. This allows us to effectively revisit
                nodes to deal with cycles.
            2.3 When Dijkstra is done running, our graph will have the optimal path to intercept the friend saved in the intercept attribute.
            2.4 With the information saved in the intercept attribute, we can backtrack to find the path taken to intercept the friend.
            2.5 The function will then return a tuple with the total cost, total time, and the path taken to intercept the friend.
        ...
        * R = number of roads, S = number of stations, L = number of locations, C = cycle_time/number of layers
        Time Complexity: O(R log L)
          The time complexity of this function can be determined by the time complexity of our preprocess function, the Dijkstra algorithm, and
          our backtracking process. Breaking it down into steps we have:
            1.  Preprocess takes O(R) time complexity to create the graph and add the edges and vertices along with necessary information
            2.  Dijkstra takes O(R log L) time complexity to run since we are using a minheap to store the vertices, where we will relax every edge
                in the graph and update the vertices in the heap, which in total will take O(R log L) (full explanation in Dijkstra).
            3.  Our backtracking process checks for the intercept attribute in the graph, where we will backtrack through the graph to find the path
                taken. This has a complexity of O(C * L) because we can only visit each vertex once, and in the worst case we visit every vertex
                in every multiverse.
          
          So, we have O(R + R log L + C * L), where we know that C can be treated as a constant as there can only be a max of 100 layers, and R >= L,
          so we can simplify this to O(R log L) time complexity.
        
        Aux Space Complexity: O(R + L)
          The function requires additional space to store the graph and the layers of the graph. The graph is created with a list of vertices and
          each vertice will have roads as edges. This means that we will have a space complexity of O(R + L) for the graph. Furthermore, running
          Dijkstra itself requires the creation of a minheap with size C*L, giving us O(L) space complexity since C is treated as a constant. Lastly,
          we have our backtracking which also requires at worst case O(C*L) complexity to store the path taken. This gives us a total aux space
          complexity of O(R + L + L + L), which we can simplify to O(R + L).
        
        Input Space Complexity: O(R)
          The input space complexity is O(R + S) since we are given the roads and stations as an input. However, we know that S (stations) has an
          upper bound of 20, so we can treat S as a constant. This means that we can simplify the input space complexity to O(R).
        
        Total Space Complexity: O(R + L)
          Taking into account our aux and input space complexity, we will get O(R + R + L), which we can just simplify to O(R + L) total space complexity.
    """
